...
import kotlinx.android.synthetic.main.activity_accepted_request.nav_side_list_id

class ActivityAcceptedRequest : BaseActivity(),NavigationView.OnNavigationItemSelectedListener {

    //global variables
    private lateinit var mRefAccepted: DatabaseReference
    lateinit var acceptedData:ArrayList<RequestAccepted>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        connectActionbar()
        acceptedData= ArrayList()
        connectDatabase()

        mRefAccepted.addValueEventListener(object: ValueEventListener
        {
            override fun onDataChange(snapshot: DataSnapshot) {
                for( data in snapshot.children)
                {
                    acceptedData.add(data.getValue(RequestAccepted::class.java)!!)
                }
                val adapterAcceptedRequest=AdapterAcceptedRequest(baseContext,R.layout.view_accepted_request,acceptedData)
                adapterAcceptedRequest.notifyDataSetChanged()
                grid_accepted_request_id.adapter=adapterAcceptedRequest
            }
    ...
    {
        val actionToggle= ActionBarDrawerToggle(this,dr_accepted_id,toolbar,R.string.drawer_open,R.string.drawer_close)
        dr_accepted_id.addDrawerListener(actionToggle)
    ...
    {
        dr_accepted_id.closeDrawer(GravityCompat.START)
    }

    override fun onBackPressed() {
        if(dr_accepted_id.isDrawerOpen(GravityCompat.START))
            closeDrawer()
        else
            super.onBackPressed()
    }
